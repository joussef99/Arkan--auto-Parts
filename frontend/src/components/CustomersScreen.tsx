...
export const CustomersScreen = () => {
  ...
  const [filter, setFilter] = useState<
    "all" | "debt" | "settled" | "credit" | "no_debt"
  >("all");
  ...
  const [activeProfileTab, setActiveProfileTab] = useState<
    "info" | "invoices" | "payments" | "statement"
  >("info");
  ...
  const handleDeleteCustomer = async (id: number) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        console.log(
          `[UI] Delete blocked for customer ID: ${id}. Reason: ${err.error}`,
        );
  ...
  return (
    <div className="flex flex-col gap-6">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-slate-500">
              متابعة الديون والمدفوعات والنشاط
            </p>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        {/* Customer List */}
        <section
          className="lg:col-span-1 bg-white rounded-2xl shadow-sm border border-slate-200 flex flex-col h-[calc(100vh-250px)]"
          aria-label="قائمة الزبائن"
        >
          <header className="p-4 border-b border-slate-100 font-bold text-slate-700 flex justify-between items-center">
            <span>قائمة الزبائن</span>
            <span
              className="text-xs bg-slate-100 px-2 py-1 rounded text-slate-500"
              aria-label={`عدد الزبائن: ${customers.length}`}
            >
              {customers.length}
            </span>
          </header>
          <div className="flex-1 overflow-y-auto p-2" role="list">
            {customers.map((c) => (
              <article
                ...
                aria-label={`${c.name}, الرصيد: ${c.current_balance} دينار`}
                className={`w-full text-right p-4 rounded-xl mb-2 transition-all flex flex-col gap-1 border cursor-pointer focus:outline-none focus:ring-2 focus:ring-emerald-500 ${
                ...
                <div className="flex justify-between items-center group">
                  ...
                  >
                    <span
                      ...
                      role="status"
                      aria-label={`حالة الرصيد: ${c.current_balance > 0 ? `${c.current_balance} دينار مدين` : "خالص"}`}
                    >
                      {c.current_balance > 0
                    ...
                    <button
                      ...
                      className="p-1 text-red-400 hover:text-red-600 hover:bg-red-50 rounded transition-all disabled:opacity-30 focus:outline-none focus:ring-2 focus:ring-red-500"
                      title="حذف"
                    >
          ...
          {selectedCustomer ? (
              ...
              <div className="p-6 border-b border-slate-100 bg-slate-50/50 rounded-t-2xl">
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mt-8">
                  <div className="bg-white p-4 rounded-xl border border-slate-100 shadow-sm">
                    ...
                    <p
                      className={`text-xl font-black ${selectedCustomer.current_balance > 0 ? "text-red-600" : "text-emerald-600"}`}
                    >
                  ...
                  <div className="bg-white p-4 rounded-xl border border-slate-100 shadow-sm">
                    ...
                    <p className="text-xl font-black text-slate-900">
                      {selectedCustomer.stats?.total_purchases || 0} د.ل
                    </p>
                  </div>
                  <div className="bg-white p-4 rounded-xl border border-slate-100 shadow-sm">
                    <p className="text-xs text-slate-500 mb-1">
                    ...
                    <p className="text-xl font-black text-emerald-600">
                      {selectedCustomer.stats?.total_paid || 0} د.ل
                    </p>
                  </div>
                  <div className="bg-white p-4 rounded-xl border border-slate-100 shadow-sm">
                    <p className="text-xs text-slate-500 mb-1">عدد الفواتير</p>
                    <p className="text-xl font-black text-slate-900">
                      {selectedCustomer.stats?.total_invoices || 0}
                    </p>
                  </div>
                </div>
              </div>
                ...
                {activeProfileTab === "info" && (
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                      ...
                      <div className="space-y-2">
                        ...
                        <div className="flex justify-between py-2 border-b border-slate-50">
                          ...
                          <span className="font-medium text-red-600">
                            {selectedCustomer.credit_limit} د.ل
                          </span>
                        </div>
                      </div>
                    </div>
                    <div className="space-y-4">
                      ...
                      <div className="p-4 bg-slate-50 rounded-xl text-slate-600 min-h-25">
                            ...
                            return (
                              ...
                              >
                                ...
                                <td className="p-3 font-medium">
                                  {entry.type === "invoice"
                                    ? `فاتورة مبيعات #${entry.ref_id}`
                                    : `دفعة نقدية #${entry.ref_id}`}
                                </td>
                        ...
                        <tbody className="divide-y divide-slate-50">
                          {customerInvoices.map((inv) => (
                            ...
                            >
                              ...
                              <td className="p-3 font-bold text-red-600">
                                {inv.total_amount - inv.paid_amount} د.ل
                              </td>
                            </tr>
                          ))}
            ...
            <div className="flex-1 flex flex-col items-center justify-center text-slate-400 p-12 text-center">
              ...
              <p className="max-w-xs mt-2">
                يمكنك البحث عن الزبائن من القائمة الجانبية أو إضافة زبون جديد
                للبدء.
              </p>
            </div>
          )}
            ...
            >
              ...
              <form onSubmit={handleAddPayment} className="p-6 space-y-6">
                <div className="p-4 bg-emerald-50 rounded-xl border border-emerald-100">
                  ...
                  <p className="font-black text-xl text-emerald-900">
                    {selectedCustomer?.current_balance} د.ل
                  </p>
                </div>
