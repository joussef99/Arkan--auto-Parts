...
import { isDesktop } from "../utils/desktopApi";
...
export const SettingsScreen: React.FC = () => {
  ...
  const [currentControllerPassword, setCurrentControllerPassword] =
    useState("");
  ...
  const showDesktopMessage = (text: string, type: "success" | "error") => {
  ...
  const backupNow = async () => {
    ...
    try {
      ...
      if (result.success) {
        showDesktopMessage("تم إنشاء نسخة احتياطية بنجاح", "success");
      } else {
        showDesktopMessage("فشل إنشاء نسخة احتياطية", "error");
      }
    } catch {
      showDesktopMessage("تعذر إنشاء النسخة الاحتياطية", "error");
    } finally {
  ...
  const restoreBackup = async () => {
    if (!window.desktopAPI) return;
    if (!confirm("سيتم استبدال قاعدة البيانات الحالية. هل تريد المتابعة؟")) {
      return;
    }

    setDesktopBusy(true);
    try {
      const result = await window.desktopAPI.restoreDatabase();
      if (!result.canceled) {
        showDesktopMessage("تم استعادة النسخة الاحتياطية بنجاح", "success");
        await fetchData();
      }
    } catch {
      showDesktopMessage("فشل استعادة النسخة الاحتياطية", "error");
    } finally {
      setDesktopBusy(false);
    }
  };
  ...
  const exportDatabase = async () => {
    ...
    try {
      ...
      if (!result.canceled) {
        showDesktopMessage("تم تصدير قاعدة البيانات", "success");
      }
    } catch {
      showDesktopMessage("فشل تصدير قاعدة البيانات", "error");
    } finally {
      setDesktopBusy(false);
    }
  };
  ...
  const importDatabase = async () => {
    ...
    setDesktopBusy(true);
    try {
      const result = await window.desktopAPI.importDatabase();
      if (!result.canceled) {
        showDesktopMessage("تم استيراد قاعدة البيانات بنجاح", "success");
        await fetchData();
      }
    } catch {
      showDesktopMessage("فشل استيراد قاعدة البيانات", "error");
    } finally {
      setDesktopBusy(false);
    }
  };
  ...
  const fetchData = async () => {
    setLoading(true);
    try {
      const [settingsRes, catRes, brandRes, subscriptionRes] =
        await Promise.all([
          fetch("/api/settings"),
      ...
      <div className="w-64 flex flex-col gap-2">
        ...
        {isDesktop() && (
          <button
            ...
            className={`flex items-center gap-3 px-6 py-4 rounded-2xl font-bold transition-all ${activeTab === "desktop" ? "bg-emerald-600 text-white shadow-lg shadow-emerald-200" : "text-slate-500 hover:bg-slate-100"}`}
          >
            <Database size={20} />
      ...
      <div className="flex-1 bg-white p-10 rounded-3xl shadow-sm border border-slate-100 relative">
        ...
        {activeTab === "general" && (
          <motion.div
            ...
            className="space-y-6"
          >
            <h3 className="text-xl font-black text-slate-900 mb-8 border-b pb-4">
          ...
          >
            ...
            <div className="grid grid-cols-2 gap-4">
              {categories.map((cat) => (
                <div
                  key={cat.id}
                  className="p-4 bg-white border border-slate-100 rounded-2xl flex justify-between items-center group hover:border-emerald-200 transition-all"
                >
          ...
          >
            ...
            <div className="bg-slate-50 p-6 rounded-2xl mb-8">
              ...
              <div className="grid grid-cols-2 gap-4 mb-4">
                ...
                <input
                  ...
                  onChange={(e) =>
                    setNewBrand({ ...newBrand, name_en: e.target.value })
                  }
                />
              </div>
              <input
            ...
            <div className="grid grid-cols-3 gap-4">
              {brands.map((brand) => (
                <div
                  key={brand.id}
                  className="p-4 bg-white border border-slate-100 rounded-2xl flex flex-col items-center gap-3 text-center"
                >
          ...
          >
            ...
            <div className="flex flex-col items-center justify-center py-12 text-slate-400 border-2 border-dashed border-slate-100 rounded-3xl">
              ...
              <p className="mt-4 font-bold">
                هذه الوحدة قيد التطوير في النسخة v2.1
              </p>
              <p className="text-xs mt-1">
                يمكنك حالياً استخدام حساب arkan الافتراضي
              </p>
            </div>
          </motion.div>
        )}
              ...
              <div className="space-y-2">
                ...
                <input
                  ...
                  onChange={(e) =>
                    setSettings({
                      ...settings,
                      main_cashbox_name: e.target.value,
                    })
                  }
                />
              </div>
          ...
          >
            ...
            <div className="p-6 rounded-2xl border border-slate-200 bg-slate-50 space-y-3">
              ...
              <div className="flex items-center justify-between">
                ...
                <span
                  className={`px-3 py-1 rounded-full text-xs font-bold ${subscription?.is_active ? "bg-emerald-100 text-emerald-700" : "bg-red-100 text-red-700"}`}
                >
            ...
            <div className="p-6 rounded-2xl border border-slate-200 bg-white space-y-4">
              <h4 className="font-black text-slate-800">
                تمديد الاشتراك (يتطلب كلمة مرور التحكم)
              </h4>
              <div className="grid grid-cols-2 gap-4">
                <input
                  type="number"
                ...
                onClick={async () => {
                  try {
                    const res = await fetch(
                      "/api/settings/subscription/extend",
                      {
                        method: "POST",
                        headers: { "Content-Type": "application/json" },
                        body: JSON.stringify({
                    ...
                    if (!res.ok || !data.success) {
                      setMessage({
                        text: data.error || "فشل تمديد الاشتراك",
                        type: "error",
                      });
                      return;
                    }
                    setMessage({
                      text: "تم تمديد الاشتراك بنجاح",
                      type: "success",
                    });
            ...
            <div className="p-6 rounded-2xl border border-slate-200 bg-white space-y-4">
              ...
              <button
                onClick={async () => {
                  try {
                    const res = await fetch(
                      "/api/settings/subscription/change-password",
                      {
                        method: "POST",
                        headers: { "Content-Type": "application/json" },
                        body: JSON.stringify({
                          currentPassword: currentControllerPassword,
                          newPassword: newControllerPassword,
                        }),
                      },
                    );
                    const data = await res.json();
                    if (!res.ok || !data.success) {
                      setMessage({
                        text: data.error || "فشل تغيير كلمة المرور",
                        type: "error",
                      });
                      return;
                    }
                    setMessage({
                      text: "تم تغيير كلمة مرور التحكم بنجاح",
                      type: "success",
                    });
            ...
            <div className="p-6 rounded-2xl border border-red-200 bg-red-50 space-y-4">
              <h4 className="font-black text-red-800">
                إنهاء الاشتراك فوراً (تحكم كامل)
              </h4>
              <p className="text-sm text-red-700">
                هذا الإجراء سينهي الاشتراك الحالي فوراً ويوقف الوصول حتى يتم
                تجديد فترة جديدة.
              </p>
              ...
              <button
                onClick={async () => {
                  if (!confirm("سيتم إنهاء الاشتراك فوراً. هل أنت متأكد؟")) {
                    return;
                  }

                  try {
                    const res = await fetch(
                      "/api/settings/subscription/reset",
                      {
                        method: "POST",
                        headers: { "Content-Type": "application/json" },
                        body: JSON.stringify({
                    ...
                    if (!res.ok || !data.success) {
                      setMessage({
                        text: data.error || "فشل إنهاء الاشتراك",
                        type: "error",
                      });
                      return;
                    }
                    setMessage({
                      text: "تم إنهاء الاشتراك فوراً",
                      type: "success",
                    });
                    setSubscriptionPassword("");
                    setRenewDays(30);
              ...
              {showRenewPrompt && (
                ...
                >
                  <motion.div
                    ...
                    className="w-full max-w-md rounded-3xl border border-amber-200 bg-white p-8 shadow-2xl"
                  >
                    <h4 className="text-2xl font-black text-amber-700 mb-3">
                      تم إنهاء الاشتراك
                    </h4>
                    <p className="text-slate-600 mb-4">
                      أدخل مدة الاشتراك الجديدة وكلمة مرور التحكم لتفعيل
                      الاشتراك مباشرة.
                    </p>
                    ...
                    <div className="grid grid-cols-1 gap-3 mb-5">
                      <input
                        ...
                        className="px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl outline-none"
                        placeholder="عدد أيام الاشتراك الجديدة"
                      />
                      <input
                        type="password"
                        value={renewPassword}
                      ...
                      <button
                        onClick={async () => {
                          ...
                          try {
                            ...
                            setRenewPassword("");
                            setMessage({
                              text: "تم تفعيل الاشتراك للفترة الجديدة بنجاح",
                              type: "success",
                            });
                          } catch {
                            setMessage({
                        ...
                        disabled={renewing}
                        className="flex-1 py-3 rounded-xl bg-emerald-600 text-white font-bold hover:bg-emerald-700 transition-colors disabled:opacity-60 disabled:cursor-not-allowed"
                      >
                        {renewing ? "جاري التفعيل..." : "تمديد الاشتراك الآن"}
                      </button>
                      <button
                        onClick={() => {
                          setShowRenewPrompt(false);
          ...
          >
            ...
            <div className="grid grid-cols-2 gap-4">
              <button
                ...
                className="px-5 py-4 rounded-2xl bg-emerald-600 text-white font-bold hover:bg-emerald-700 disabled:opacity-60 flex items-center justify-center gap-2"
              >
                <Database size={18} />
                إنشاء نسخة احتياطية
              </button>
              <button
                ...
                disabled={desktopBusy}
                className="px-5 py-4 rounded-2xl bg-amber-500 text-white font-bold hover:bg-amber-600 disabled:opacity-60 flex items-center justify-center gap-2"
              >
                <Download size={18} />
                استعادة نسخة
              </button>
              <button
                ...
                disabled={desktopBusy}
                className="px-5 py-4 rounded-2xl bg-slate-900 text-white font-bold hover:bg-slate-800 disabled:opacity-60 flex items-center justify-center gap-2"
              >
                <Upload size={18} />
              ...
              <button
                ...
                disabled={desktopBusy}
                className="px-5 py-4 rounded-2xl bg-blue-600 text-white font-bold hover:bg-blue-700 disabled:opacity-60 flex items-center justify-center gap-2"
              >
                <Download size={18} />
            ...
            <div className="flex gap-4">
              <button
                onClick={async () => {
                  const paths = await window.desktopAPI?.getDesktopPaths();
                  if (paths?.backups) {
                    await window.desktopAPI?.openFolder(paths.backups);
                  }
                }}
                className="px-5 py-3 rounded-xl bg-slate-100 text-slate-700 font-bold hover:bg-slate-200 flex items-center gap-2"
              >
                <FolderOpen size={18} />
                فتح مجلد النسخ
              </button>
              <button
                onClick={async () => {
                  await window.desktopAPI?.openLogs();
                }}
                className="px-5 py-3 rounded-xl bg-slate-100 text-slate-700 font-bold hover:bg-slate-200 flex items-center gap-2"
              >
                <FolderOpen size={18} />
