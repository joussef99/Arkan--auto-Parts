...
import { savePdfBytes } from "../utils/desktopApi";
...
export const InvoiceScreen: React.FC<InvoiceScreenProps> = ({
  ...
  const generateWhatsAppSummary = () => {
    ...
    window.open(
      `https://wa.me/${customer?.phone || ""}?text=${encodedSummary}`,
      "_blank",
    );
  };
  ...
  const exportAsPDF = () => {
    ...
    toJpeg(invoiceRef.current, { quality: 0.95, backgroundColor: "#fff" })
      .then((dataUrl) => {
        ...
        void savePdfBytes(bytes, `${invoiceNumber}.pdf`);
      })
      .catch((err) => {
  ...
  const generateImage = () => {
    if (!invoiceRef.current) return;
    toPng(invoiceRef.current, { quality: 1, backgroundColor: "#fff" }).then(
      (dataUrl) => {
      ...
      <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-200">
        ...
        <div className="flex flex-wrap items-center justify-between gap-4">
          <div className="flex items-center gap-4">
            <div
              className={`p-3 rounded-full ${isWalkIn ? "bg-emerald-100 text-emerald-600" : "bg-blue-100 text-blue-600"}`}
            >
              <User size={24} />
            </div>
            <div>
                      ...
                      >
                        ...
                        <div className="flex items-center gap-4">
                          ...
                          <div className="text-left">
                            ...
                            <div
                              className={`font-bold ${part.quantity > 0 ? "text-slate-600" : "text-red-600"}`}
                            >
                              {part.quantity}
                            </div>
                          </div>
                ...
                <tr>
                  ...
                  >
                    <div className="flex flex-col items-center gap-2">
                      ...
                      <p className="text-xs">
                        استخدم وحدة البحث لإضافة قطع غيار للفاتورة
                      </p>
                    </div>
                  </td>
                </tr>
            ...
            <div className="space-y-6">
              ...
              <div className="space-y-4">
                ...
                <div className="p-4 bg-slate-50 rounded-xl border border-slate-100 flex justify-between items-center">
                  ...
                  <span
                    className={`text-xl font-black ${remainingBalance > 0 ? "text-red-600" : "text-emerald-600"}`}
                  >
          ...
          <div className="mt-auto pt-4 border-t border-slate-100 space-y-3">
            ...
            <button
              onClick={() => {
                if (
                  window.confirm(
                    "هل أنت متأكد من إلغاء الفاتورة؟ سيتم مسح جميع البيانات.",
                  )
                ) {
                  clearInvoice();
                  setCustomer(null);
        ...
        >
          <div className="flex justify-between items-start border-b-4 border-slate-900 pb-8 mb-8">
            <div>
              ...
              <p className="text-lg text-slate-600">
                طرابلس، ليبيا - شارع قطع الغيار
              </p>
              <p className="text-lg text-slate-600">
                هاتف: 091-0000000 / 092-0000000
              </p>
            </div>
            <div className="text-left">
              <h2 className="text-3xl font-black text-slate-400 uppercase mb-2">
                فاتورة مبيعات
              </h2>
          ...
          <div className="grid grid-cols-2 gap-10 mb-10">
            <div className="bg-slate-50 p-6 rounded-2xl border border-slate-200">
              <h3 className="font-black text-slate-900 mb-4 border-b border-slate-200 pb-2">
                بيانات الزبون
              </h3>
              <div className="space-y-2 text-lg">
                <p>
                  <span className="font-bold">الاسم:</span>{" "}
                  {isWalkIn ? "زبون نقدي" : customer?.name}
                </p>
                {!isWalkIn && (
                  <>
                    <p>
            ...
            <div className="bg-slate-50 p-6 rounded-2xl border border-slate-200">
              ...
              <div className="space-y-2 text-lg">
                <p>
                  ...
                  {paymentType === "cash" ? "نقدي" : "آجل"}
                </p>
                <p>
                  <span className="font-bold">حالة الفاتورة:</span>{" "}
                  {remainingBalance <= 0 ? "مدفوعة بالكامل" : "متبقي ذمة"}
                </p>
              </div>
            </div>
          </div>
          ...
          <table className="w-full mb-10 border-collapse">
            <thead>
              <tr className="bg-slate-900 text-white">
                <th className="p-4 text-right border border-slate-900">
                  الصنف
                </th>
                <th className="p-4 text-center border border-slate-900">OEM</th>
                <th className="p-4 text-center border border-slate-900">
                  الكمية
                </th>
                <th className="p-4 text-center border border-slate-900">
                  السعر
            ...
            <tbody>
              {items.map((item, idx) => (
                ...
                >
                  <td className="p-4 border border-slate-200 font-bold">
                    {item.part_name_ar}
                  </td>
                  <td className="p-4 border border-slate-200 text-center font-mono">
                    {item.oem_number}
                  </td>
          ...
          <div className="mt-20 pt-10 border-t border-slate-200 text-center text-slate-400 text-sm">
            ...
            <p className="mt-2 italic">
              صدرت هذه الفاتورة إلكترونياً بواسطة نظام أركان لإدارة المحلات
            </p>
          </div>
        </div>
      </div>
